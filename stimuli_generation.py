...
def get_rsvp(fixation, rsvp_files, frame_list, source_folder, quad_map):
    lookup_conds = {}
    frames = [[fixation] + [stimuli.Rectangle(size=(300,200), colour=C_WHITE, position=quad_map[quadrant]) for quadrant in quad_map.keys()]]
    for frame in frame_list:
        stims = [fixation]
        for quadrant in frame:
            picture_file = rsvp_files.pop()
            stims.append(stimuli.Picture(os.path.join(source_folder, picture_file), quad_map[quadrant]))
            lookup_conds[picture_file] = {'quad': quadrant, 'total_stims': len(frame)}
        for quadrant in quad_map.keys():
            if quadrant not in frame:
                stims.append(stimuli.Rectangle(size=(300,200), colour=C_WHITE, position=quad_map[quadrant]))
        frames.append(stims)
    frames.append([fixation] + [stimuli.Rectangle(size=(300,200), colour=C_WHITE, position=quad_map[quadrant]) for quadrant in quad_map.keys()])
...
def generate_stims(trial_settings, pictures_list, source_folder, quad_map, fixation):
    ...
    rsvp_frames, lookup_conds = get_rsvp(fixation, rsvp_files, trial_settings['rsvp'], source_folder, quad_map)
